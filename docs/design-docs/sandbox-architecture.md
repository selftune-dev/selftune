...
make sandbox-install        # Empty-state install validation
...
# Utility
make sandbox-reset          # Remove persisted Claude sandbox HOME
make sandbox-reset-state    # Remove selftune state, keep Claude auth
make sandbox-shell          # Provisioned Claude Code container
make sandbox-shell-empty    # Blank Claude Code container
make sandbox-shell-empty-workspace # Blank container with selftune wired to /app
