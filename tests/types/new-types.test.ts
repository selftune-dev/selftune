...
describe("new type definitions", () => {
  test("GradingExpectation accepts optional score and source", () => {
    ...
    const expWithScore: GradingExpectation = {
      text: "test", passed: true, evidence: "found",
      score: 0.85, source: "pre-gate",
    };
  ...
  test("GradingSummary accepts optional mean_score and score_std_dev", () => {
    ...
    const summaryWithScores: GradingSummary = {
      passed: 5, failed: 1, total: 6, pass_rate: 0.83,
      mean_score: 0.78, score_std_dev: 0.12,
    };
  ...
  test("FailurePattern accepts optional feedback field", () => {
    const pattern: FailurePattern = {
      ...
      missed_queries: ["q1"], frequency: 1, sample_sessions: [], extracted_at: new Date().toISOString(),
    };
  ...
  test("InvocationTypeScores has all four dimensions", () => {
    const scores: InvocationTypeScores = {
      ...
      implicit: { passed: 4, total: 5, pass_rate: 0.80 },
  ...
  test("ParetoCandidate compiles with all required fields", () => {
    const candidate: ParetoCandidate = {
      proposal: {
        proposal_id: "evo-1", skill_name: "test", skill_path: "/test",
        original_description: "old", proposed_description: "new", rationale: "r",
        failure_patterns: [], eval_results: {
      ...
      validation: {
        proposal_id: "evo-1", before_pass_rate: 0.5, after_pass_rate: 0.8,
        improved: true, regressions: [], new_passes: [], net_change: 0.3,
      },
      invocation_scores: {
        explicit: { passed: 5, total: 6, pass_rate: 0.83 },
        implicit: { passed: 4, total: 5, pass_rate: 0.80 },
  ...
  test("backward compatibility: existing types work without new fields", () => {
    ...
    const oldPattern: FailurePattern = {
      pattern_id: "fp-1", skill_name: "s", invocation_type: "explicit",
      missed_queries: [], frequency: 0, sample_sessions: [], extracted_at: "",
    };
