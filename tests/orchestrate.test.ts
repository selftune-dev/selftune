...
describe("formatOrchestrateReport", () => {
  test("includes dry-run mode banner", () => {
    ...
    expect(report).toContain("--auto-approve");
  });

  test("includes auto-approve mode banner", () => {
  ...
  test("includes watch results when skills were watched", () => {
    const result = makeOrchestrateResult({
      candidates: [
        {
          skill: "RecentSkill",
    ...
    expect(report).toContain("pass_rate=0.90");
  });
